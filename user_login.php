<?php
    ...
    $check_u = "select * from user where uname ='$uname'";
    $res_u = mysqli_query($con,$check_u) or die(mysqli_error($con));
    $check_p = "select * from user where uname = '$uname' and psw = '$psw'";
    $res_p = mysqli_query($con,$check_p) or die(mysqli_error($con));

    if(mysqli_num_rows($res_u)==0)
    {
      echo "<script type='text/javascript'>alert('Incorrect uname!!');</script>";
      header("refresh: 0.01; url=user_login.html");
    }

    if(mysqli_num_rows($res_p)==0)
    {
      echo "<script type='text/javascript'>alert('Incorrect psw!!');</script>";
      header("refresh: 0.01; url=user_login.html");
    }
...
<form action="user_login.php" method="post">
  ...
  <div class="container" style="text-align:center;">
    ...
    <input type="psw" placeholder="Enter psw" name="psw" required><br><br>        
